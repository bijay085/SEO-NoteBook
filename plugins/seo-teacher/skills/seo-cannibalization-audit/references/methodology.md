...
7. **Spearman** rank correlation ≤ `handoff_min_anticorrelation` (−0.30). Rank, not Pearson: weekly
...
- 10–25% → review the lost queries, then 301
...
flat or declining, the action becomes STAGED: audit → port → canonical → observe 4–6 weeks → redirect
...
`handoff_min_anticorrelation` −0.30→−0.20, `handoff_min_pre_clicks` 5→3,
