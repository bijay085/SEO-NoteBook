...
- Set a date window (default last ~3–6 months; GSC lags ~2–3 days, so end ~2 days before today).
...
1. **"This is a ranking problem."** First separate the two failure modes (see *Reading the data*).
   A page at average position 4–6 is **not** a ranking failure — if its CTR is near-zero it's a
...
- Position ≤ ~8 but CTR well under ~1% on big impressions → **zero-click SERP**: AI Overviews,
  ...
  capture* (tight 40–55-word answer blocks, FAQ schema) or *pivoting to query types that still get
...
**Quick wins**: tool/calculator/comparison pages and money pages sitting at position 11–20 with real
