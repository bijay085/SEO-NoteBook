...
```mermaid
flowchart TD
    A["Query network"] --> B["Entity + attributes + context"]
    B --> C{"Does it deserve a page?"}
    C -->|Yes| D["Select page type and function"]
    C -->|No| E["Merge, consolidate, or omit"]
    D --> F["Map intent to centerpiece"]
    F --> G["Add supporting visual and textual components"]
    G --> H["Connect through the topical and link network"]
```
...
```mermaid
flowchart TD
    A["Query cluster"] --> B["Meaning and task"]
    B --> C["Page eligibility"]
    C --> D["Page type"]
    D --> E["Primary function"]
    E --> F["Centerpiece"]
    F --> G["Supporting components"]
    G --> H["Text, evidence, and links"]
```
