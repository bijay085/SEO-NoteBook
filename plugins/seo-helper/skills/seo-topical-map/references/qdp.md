...
1. mark **different entity** → 2. resolve **parent to child** → 3. **similarity** →
...
warrants a dedicated page. Use it on every 2 to 3/4 candidate.
...
| #3 low similarity (vs parent) | ontology parent to child + similarity: shared head term, subset relation, salience proximity to the parent |
...
| local seo cost | weak to moderate (geo shifts intent) | ✗ | ✓ | ? data | same; local is often promoted by the SERP test |
| seo audit cost | moderate (audit is a distinct deliverable) | ✗ to ~ | ✓ | ? data | borderline → SERP decides |
