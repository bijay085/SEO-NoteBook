#!/usr/bin/env python3
"""Shared SEO report branding for the Initial Analysis skill.
...
combine.py so every section shares one look. Mirrors the periodic-audit brand_lib so
both engagement phases render identically.

    import brand_lib as B
    cfg = B.load_config("config.json")
    html = B.shell("KG-Equivalent EAV", inner_html, cfg)   # standalone report
...
def footer(cfg):
    ...
    return (f"<div class='report-footer'><span>&copy; {b['agency']}</span>"
            f"<span>{cfg.get('client',{}).get('name','')} &middot; Initial Analysis</span></div>")
